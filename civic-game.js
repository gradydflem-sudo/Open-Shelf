(function () {
  ...
  const IDEOLOGIES = [
    { id: "progressive", name: "Progressive", score: -18, groups: { collegeStudents: 3, environmentalVoters: 3, unionMembers: 2, ruralVoters: -2, religiousVoters: -2 } },
    { id: "liberal", name: "Liberal", score: -10, groups: { urbanProfessionals: 2, teachers: 2, immigrantCommunities: 2, smallBusiness: -1 } },
    { id: "moderate", name: "Moderate", score: 0, groups: { suburbanModerates: 3, parents: 2, seniors: 1, collegeStudents: -1 } },
    { id: "conservative", name: "Conservative", score: 14, groups: { religiousVoters: 3, seniors: 2, smallBusiness: 2, collegeStudents: -3, environmentalVoters: -2 } },
    { id: "populist", name: "Populist", score: 7, groups: { workingClass: 2, ruralVoters: 2, veterans: 2, urbanProfessionals: -2 } },
    { id: "libertarian", name: "Libertarian", score: 9, groups: { smallBusiness: 3, urbanProfessionals: 1, publicSectorWorkers: -2, teachers: -2 } },
    { id: "green", name: "Green", score: -22, groups: { environmentalVoters: 5, collegeStudents: 3, coastalResidents: 2, industrialWorkers: -2, ruralVoters: -1 } },
    { id: "socialist", name: "Socialist", score: -24, groups: { unionMembers: 4, workingClass: 3, collegeStudents: 2, smallBusiness: -4, suburbanModerates: -2 } },
    { id: "nationalist", name: "Nationalist", score: 20, groups: { veterans: 4, ruralVoters: 3, industrialWorkers: 2, immigrantCommunities: -4, urbanProfessionals: -3 } },
    { id: "technocratic", name: "Technocratic", score: -2, groups: { urbanProfessionals: 3, publicSectorWorkers: 2, suburbanModerates: 2, ruralVoters: -1 } }
  ];
  ...
  function migrateState(saved) {
    ...
    saved.candidate.officeId = saved.candidate.officeId || scenario?.officeId || "provincialAssembly";
    ...
    saved.career = saved.career || {
      ...
      legacy: "Unwritten",
      skills: buildStartingSkills(byId(DATA.backgrounds, saved.candidate.backgroundId))
    };
  ...
  function startCampaign(formData) {
    const scenario = byId(DATA.scenarios, formData.get("scenario")) || DATA.scenarios[0];
    const party = byId(DATA.parties, formData.get("party"));
    ...
    const ideology = byId(IDEOLOGIES, formData.get("ideology"));
    ...
    state = {
      ...
      candidate: {
        ...
        officeId: scenario.officeId || "provincialAssembly",
        homeRegionId,
        topIssues
      },
      career: {
        mode: "career",
        currentOfficeId: scenario.officeId || "provincialAssembly",
        history: [],
        electionsWon: 0,
        electionsLost: 0,
    ...
    state.voters = DATA.voterGroups.map((group) => {
      ...
      return {
        ...group,
        support: clamp(group.support + partyMod + ideologyMod + traitMod + backgroundMod + styleMod + randomBetween(-1.2, 1.2), 22, 78),
    ...
    addLog(`${state.candidate.name} launched a career campaign for ${scenario.office}.`, "major");
  ...
  function scenarioOffice(scenario = byId(DATA.scenarios, state?.scenarioId)) {
    ...
    root.innerHTML = `
      <section class="civic-setup">
        <div class="setup-copy">
          ...
          <p class="civic-deck">Build a fictional political career in Columbia: campaign, win office, govern through tradeoffs, manage a team, pass laws, and decide what kind of public servant you become.</p>
          <div class="scenario-note">
            <strong>Career Mode is now the flagship path.</strong>
            <span>Start small, learn the campaign loop, then govern with staff, policy cards, budgets, factions, and monthly administration meetings.</span>
          </div>
          ${saved ? `
        ...
        <form class="setup-form" id="campaignSetupForm">
          <div class="form-grid">
            ...
            <label>
              <span>Party or faction</span>
              <select name="party">${DATA.parties.map((party) => `<option value="${party.id}">${escapeHTML(party.name)}</option>`).join("")}</select>
            </label>
            <label>
              <span>Ideology</span>
              <select name="ideology">${IDEOLOGIES.map((item) => `<option value="${item.id}">${escapeHTML(item.name)}</option>`).join("")}</select>
            </label>
            <label>
  ...
  function renderMapTab() {
    return `
      <div class="map-layout">
        <div class="region-map" aria-label="Fictional district map">
    ...
    return `
      <div class="career-layout">
        <article class="dashboard-card">
          ...
          <p><strong>${escapeHTML(state.candidate.name)}</strong> · ${escapeHTML(background?.name || "Candidate")} · ${escapeHTML(currentOffice?.name || state.candidate.office)}</p>
          <p class="card-copy">${escapeHTML(background?.text || "A public career is beginning.")}</p>
    ...
    return `
      <div class="hq-grid">
        ...
        <article class="dashboard-card">
          ...
          <p class="card-copy">${escapeHTML(advisorLine("Legislative Liaison", "The Moderate Reformers and Rural Coalition can decide close votes. Talk to them before forcing a bill."))}</p>
        </article>
        <article class="dashboard-card wide-card">
          <h2>Governing Log</h2>
          <div class="campaign-log">
